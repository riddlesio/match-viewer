...
GamePlayer = createClass(AbstractUIComponent, {
    ...
    render: function () {
        ...
        React.render(PlayerView({ state }), document.getElementById('player'));
    },
        ...
        var self = this,
            internalCallback = function () {
                return function () {
                    ...
                    var { min, max, timeStep } = self.getState().playbackTimeout,
                        timeout = (min + max) - timeStep;

                    self.timer = window.setTimeout(internalCallback, timeout);
                    callback();
                }
            }(0);

        self.timer = window.setTimeout(internalCallback, self.getState().playbackTimeout.timeStep);
    }
});
...
function triggerFullscreenChange (event) {
...
function triggerNormalscreen (event) {

    PlaybackEvent.trigger(PlaybackEvent.EXIT_FULL_SCREEN);
}

function triggerLoseVisibility (event) {
    if (document[hidden]) {
